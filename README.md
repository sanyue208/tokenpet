...
- ⚡ **实时 token（可选）**：**代理已内置进 app**（开关一开自动以隐藏进程运行，无需另开窗口）；把 LLM 请求的 base_url 指到它，头顶实时显示每次 prompt / completion 与累计 token（毫秒级；余额仍走官方账单接口）
...
**代理已内置在 app 里**：在 ⚙ 设置里开启「启用实时 token 统计」后，app 会自动以**隐藏进程**运行代理（`127.0.0.1:8787`），**无需另开窗口**；关闭开关或退出 app 会自动结束它。

> 本机未安装 Node 时可以手动启动，效果相同：
```powershell
npm run proxy
# 或指定上游与端口
